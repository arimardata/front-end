...
class StepperProjects extends React.Component {
  ...
  fetchProject = async () => {
    ...
    this.setChargesFixes(project.chargesFixes);
    this.setMateriels(project.materiels);
    this.setPersonnels(project.personnels);
    this.setPersonnelSelect(project.personnels);
  };
  setPersonnels = personnels => {
    const personnelAffecter = this.state.personnelAffecter;
    personnels.map(personnel => {
      personnelAffecter.push([
        personnel.personnelId,
        personnel.cin,
        this.getNameFromId(personnel.personnelId),
        personnel.diplome,
        personnel.qualite,
        personnel.type,
        parseInt(personnel.etape)
      ]);
    });
  ...
  setMateriels = materiels => {
    ...
    materiels.map(materiel => {
      data.push([
        materiel.materielId,
        materiel.materiel,
        parseInt(materiel.quantite),
        materiel.type,
        parseInt(materiel.etape)
      ]);
    });
    this.setState({ data });
  };
  ...
  setChargesFixes = charges => {
    ...
    charges.map(charge => {
      chargesFixes.push({
        id: charge.num,
        note: charge.note,
        montant: parseInt(charge.montant)
      });
    });
  ...
  setEtapes = etapes => {
    etapes.map(etape => {
      etape.responsable = this.getNameFromId(etape.responsable);
      etape.id = etape.etape;
    });
    this.setState({ etapes });
  };
  setPersonnelSelect = personnels => {
    const personnelSelect = this.state.personnelSelect;
    personnels.map(personnel => {
      if (!this.personnelAlreadyExist(personnel.personnelId))
        personnelSelect.push([
          personnel.personnelId,
          this.getNameFromId(personnel.personnelId)
        ]);
    });

    this.setState({ personnelSelect });
  };

  personnelAlreadyExist = id => {
    const personnelSelect = this.state.personnelSelect;
    let exist = false;
    personnelSelect.map(personnel => {
      if (personnel[0] === id) exist = true;
    });
    return exist;
  };
  ...
  handleCout = () => {
    ...
    etapes.map(etape => {
      ...
      personnelAffecter.map(personnel => {
        if (
          personnel.etape === parseInt(etape.etape) ||
          personnel.etape === parseInt(etape.id)
        ) {
